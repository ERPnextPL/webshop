...
class TestProductDataEngine(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for index, item in enumerate(item_codes, start=1):
            ...
            if not frappe.db.exists("Website Item", {"item_code": item_code}):
                create_regular_web_item(
                    item_code, item_args=item_args, web_args=web_args
                )
    ...
    def test_product_list_ordering_and_paging(self):
        ...
        engine = ProductQuery()
        result = engine.query(
            attributes={}, fields={}, search_term=None, start=0, item_group=None
        )
        ...
        # check next page
        result = engine.query(
            attributes={}, fields={}, search_term=None, start=4, item_group=None
        )
        items = result.get("items")
    ...
    def test_change_product_ranking(self):
        ...
        old_ranking = frappe.db.get_value(
            "Website Item", {"item_code": item_code}, "ranking"
        )
    ...
    def test_product_list_field_filter_builder(self):
        ...
        # Product group filters should reflect groups assigned to published products,
        # even if an Item Group page itself is hidden from website navigation.
        ...
        # 'Products' and 'Raw Material' remain available when their products are published.
        item_group_filters = field_filters[0]
        docfield = item_group_filters[0]
        valid_item_groups = item_group_filters[1]
    ...
    def test_item_group_filter_available_without_extra_field_filters(self):
        "Item group filter should remain available even if only attribute filters are configured."
        setup_webshop_settings(
            {
        ...
        self.assertEqual(len(field_filters), 1)
        self.assertEqual(field_filters[0][0].fieldname, "item_group")
        self.assertIn("Products", field_filters[0][1])
    ...
    def test_product_list_discount_filter_builder(self):
        ...
        make_web_pricing_rule(
            title=f"Test Pricing Rule for {item_code}", item_code=item_code, selling=1
        )
