...
class WebsiteItem(WebsiteGenerator):
    ...
    def get_context(self, context):
        ...
        self.set_gallery_images(context)
        ...
        context.variant_ui_mode = get_variant_ui_mode()
    ...
    def set_gallery_images(self, context):
        """Build a unified gallery for item page: slideshow + main image + Website Item attachments."""
        gallery_images = []
        seen_images = set()

        def append_image(image_url, heading=None):
            image_url = cstr(image_url).strip()
            ...
            gallery_images.append(
                {
                    "image": image_url,
                    "heading": cstr(heading)
                    or self.website_image_alt
                    or self.item_name,
                }
            )

        for slide in context.get("slides") or []:
            append_image(slide.get("image"), slide.get("heading"))

        append_image(self.website_image, self.website_image_alt or self.item_name)
        ...
        for attachment in attachments:
            if not self._is_supported_gallery_image(attachment):
                continue
            append_image(attachment.file_url, attachment.file_name)

        context.gallery_images = gallery_images

    def _is_supported_gallery_image(self, attachment):
        image_extensions = (
            ".jpg",
            ".jpeg",
            ".png",
        ...
        file_url = cstr(attachment.get("file_url") or "")
        file_type = cstr(attachment.get("file_type") or "").lower()

        if file_type.startswith("image/"):
            return True

        normalized_url = file_url.lower().split("?", 1)[0]
        return normalized_url.endswith(image_extensions)
...
def check_if_user_is_customer(user=None):
    return True if get_customer_for_user(user) else False
...
def get_variant_ui_mode(user=None):
    customer = get_customer_for_user(user)
    if not customer or customer.customer_type == "Individual":
        return "b2c"
