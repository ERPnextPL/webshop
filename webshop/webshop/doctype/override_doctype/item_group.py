...
class WebshopItemGroup(ItemGroup, WebsiteGenerator):
    ...
    def get_context(self, context):
        ...
        context.page_length = (
            cint(frappe.db.get_single_value("Webshop Settings", "products_per_page"))
            or 6
        )
        ...
        if self.slideshow:
            ...
            for index, slide in enumerate(slides):
                ...
                values[f"slide_{index + 1}_content_align"] = (
                    slide.get("content_align") or "Centre"
                )
...
def get_parent_item_groups(item_group_name, from_item=False):
    ...
    item_group = frappe.db.get_value(
        "Item Group", item_group_name, ["lft", "rgt"], as_dict=1
    )
...
def get_child_groups_for_website(item_group_name, immediate=False, include_self=False):
    ...
    return frappe.get_all(
        "Item Group", filters=filters, fields=["name", "route"], order_by="name"
    )
