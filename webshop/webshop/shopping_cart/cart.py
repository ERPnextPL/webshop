...
def get_party(user=None, create=True):
    if not user:
        ...
        if doc.doctype in ["Customer", "Supplier"]:
            if (
                ...
                and not frappe.db.exists(
                    "Portal User", {"parent": doc.name, "user": user}
                )
            ):
    ...
    elif create and not frappe.db.exists("Portal User", {"user": user}):
...
def export_cart_csv():
    ...
    # Write header
    writer.writerow(
        ["Item Code", "Item Name", "Quantity", "Unit Price", "Amount", "Notes"]
    )
    ...
    for item in quotation.get("items", []):
        writer.writerow(
            [
                item.item_code,
                item.get("web_item_name") or item.item_name,
                item.qty,
                item.rate,
                item.amount,
    ...
    writer.writerow([])
    writer.writerow(["Total", "", "", "", quotation.total])
...
def empty_cart():
    quotation = _get_cart_quotation()
    if quotation and quotation.name:
        quotation.flags.ignore_permissions = True
        quotation.delete()
